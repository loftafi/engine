...
// `TextHeight.pixels` defined as 16 pixels, this becomes the
// default height across all device types. If a device has a
...
pub const EntityWalker = struct {
    ...
    pub const ClosestLeft = struct {
        anchor: Vector,
        distance: f32,
        pub fn init(display: *Display) ClosestLeft {
        ...
        pub fn choose(self: *ClosestLeft, option: *const Entity, parent_offset: Vector) bool {
    ...
    pub const ClosestRight = struct {
        anchor: Vector,
        distance: f32,
        pub fn init(display: *Display) ClosestRight {
            var anchor: Vector = if (display.selected) |s|
        ...
        pub fn choose(self: *ClosestRight, option: *Entity, parent_offset: Vector) bool {
            const position = option.rect.move(parent_offset);
    ...
    pub const ClosestAbove = struct {
        anchor: Vector,
        distance: f32,
        pub fn init(display: *Display) ClosestAbove {
            var anchor: Vector = if (display.selected) |s|
        ...
        pub fn choose(self: *ClosestAbove, option: *const Entity, parent_offset: Vector) bool {
            const position = option.rect.move(parent_offset);
    ...
    pub const ClosestBelow = struct {
        anchor: Vector,
        distance: f32,
        pub fn init(display: *Display) ClosestBelow {
            var anchor: Vector = if (display.selected) |s|
        ...
        pub fn choose(self: *ClosestBelow, option: *Entity, parent_offset: Vector) bool {
            const position = option.rect.move(parent_offset);
...
pub fn find_under_cursor(
    ...
    var i = entities.len;
    while (i > 0) : (i -= 1) {
        const item: *Entity = entities[i - 1];
...
test "text input sizing" {
    ...
    try eq(33, @trunc(label.rect.height)); // label min height = 22, text size = `heading`=*1.5 at double screen density
    try eq(33 + 60, @trunc(panel.rect.height)); // Panel has two labes
}
...
const seconds = Animator.seconds;
